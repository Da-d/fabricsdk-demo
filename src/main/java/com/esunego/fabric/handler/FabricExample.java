...
public class FabricExample {
    private Logger logger = LoggerFactory.getLogger(FabricExample.class);

    @Autowired
    private ConfigUtil configUtil;

    private String ymlFileContent;
    private String channelName;
    private String chaincodeName;
    private String accessKey;
    private Map<String, HFClient> clientMap;
    private Map<String, Channel> channelMap;
    private FabricExample() {
        setConfigCtx(new ConfigUtil().getConfigContents());
        channelMap = new HashMap<>();
        clientMap = new HashMap<>();
    }

    public void setConfigCtx(String configContent) {
        if (configContent == null || configContent.equals("")) {
            logger.error("config path is empty! please input correct ymal path!");
        }
        this.ymlFileContent = configContent;
    ...
    public FabricExample setConfigPath(String configPath){
        this.ymlFileContent=configPath;
        return this;
    }

    public FabricExample setChaincodeName(String chaincodeName) {
        this.chaincodeName=chaincodeName;
        return this;
    }

    public FabricExample setChannelName(String channelName) {
        this.channelName=channelName;
        return this;
    }

    public FabricExample setAccessKey(String accesskey) {
    ...
    private NetworkConfig loadfromYamlFile(String fileName) {
        try {
            return NetworkConfig.fromYamlStream(new ByteArrayInputStream(fileName.getBytes()));
    ...
    private CryptoSuite getCryptoSuite() throws IllegalAccessException, InstantiationException, ClassNotFoundException, org.hyperledger.fabric.sdk.exception.CryptoException, org.hyperledger.fabric.sdk.exception.InvalidArgumentException, NoSuchMethodException, java.lang.reflect.InvocationTargetException, FileNotFoundException{
        CryptoSuite cs = null;
        Map<String, Object> map = new Yaml().load(this.ymlFileContent);
    ...
    private Channel getChannel(String accessKey, HFClient client) {
        ...
        if (channel == null) {
            synchronized (channelMap) {
                ...
                NetworkConfig networkConfig = loadfromYamlFile(this.ymlFileContent);
                ...
                try {
                    networkConfig.getOrdererNames().forEach(item -> {
                        try {
                            ...
                            p.setProperty("clientCertFile", GetTlsCert(ymlFileContent, "ordererorg"));
                            p.setProperty("clientKeyFile",  GetTlsKey(ymlFileContent, "ordererorg"));
                    ...
                    networkConfig.getPeerNames().forEach(item -> {
                        try {
                            ...
                            p.setProperty("hostnameOverride", item);
                            p.setProperty("clientCertFile", GetTlsCert(ymlFileContent, orgId));
                            p.setProperty("clientKeyFile",  GetTlsKey(ymlFileContent, orgId));
    ...
    private String getCryptoPath(String configContent, String orgId) {
    ...
    private String GetTlsCert(String configContent, String orgId){
        String msp = getCryptoPath(configContent, orgId);
        int index = msp.lastIndexOf("msp");
        String ret = msp.substring(0, index)+"tls_server.crt";
        logger.debug("tls cert for " + orgId +",path:"+ ret);
        return ret;
    }

    private String GetTlsKey(String configContent, String orgId){
        String msp = getCryptoPath(configContent, orgId);
        int index = msp.lastIndexOf("msp");
        String ret = msp.substring(0, index)+"tls_server.key";
    ...
    private FabricUser genFabricUser(String accessKey) {
        ...
        String msp = getCryptoPath(this.ymlFileContent, accessKey);
    ...
    private String extractPemString(String path, String sub){
        ...
        return configUtil.getContent(path);
    }
